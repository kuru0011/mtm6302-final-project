...
// function to display greeting according to the time
function greeting() {
    if (h < 11) {
...
// fetching the image from NASA website
fetch('https://api.nasa.gov/planetary/apod?api_key=NAPGpwPBThXaqqtNGwOUfz7hg3TvtYKMvBL19SEl')
    ...
    .then(function(imageData){
        ...
        //displaying an image when its a video
        if (imageData.media_type === 'video'){
            document.querySelector('img').setAttribute = ('src', '/images/placeholder.jpg')
        } 
        else {
    ...
    // function checking if the seconds display option is stored in the local storage
    ...
    checkLocalStorage()
     ...
     // function checking if the date display option is stored in the local storage
    ...
    // function to add zero in front of the numbers when its a single digit 
    function addZero(i){
        if (i < 10){
            i = '0' + i
        }
